...
class CansaCentre_Customizer_Utilities {
	/**
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					...
					$title = sprintf( esc_html__( 'ID: %s No Title', 'cansa-centre' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			...
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'cansa-centre' ),
			'entire-site'  => esc_html__( 'Entire Site', 'cansa-centre' ),
		);
 ...
 * Initialize class
 */
$deneb_utilities = new CansaCentre_Customizer_Utilities();
