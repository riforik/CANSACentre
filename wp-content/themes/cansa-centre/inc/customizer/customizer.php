...
class CansaCentre_Customizer {
	...
	public function register_options( $wp_customize ) {
		...
		$section_visibility = CansaCentre_Customizer_Utilities::section_visibility();

		$section_visibility['excluding-home'] = esc_html__( 'Excluding Homepage', 'cansa-centre' );
		...
		$wp_customize->add_section( new CansaCentre_Upsell_Section( $wp_customize, 'upsell_section',
			array(
				'title'           => esc_html__( 'CansaCentre Pro Available', 'cansa-centre' ),
				'url'             => 'https://fireflythemes.com/themes/cansa-centre-pro',
	...
	public function preview_js() {
		...
		wp_enqueue_script( 'cansa-centre-customizer', get_template_directory_uri() . '/js/customizer-preview' . $min . '.js', array( 'customize-preview' ), deneb_get_file_mod_date( '/js/customizer-preview' . $min . '.js' ), true );
	}
	...
	public function customize_control_js() {
		...
		// Enqueue Select2.
		wp_enqueue_script( 'cansa-centre-select2-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/select2' . $min . '.js', array( 'jquery' ), '4.0.13', true );
		wp_enqueue_style( 'cansa-centre-select2-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/select2' . $min . '.css', array(), '4.0.13', 'all' );
		
		// Enqueue Custom JS and CSS.
		wp_enqueue_script( 'cansa-centre-custom-controls-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/customizer' . $min . '.js', array( 'jquery', 'jquery-ui-core', 'cansa-centre-select2-js' ), deneb_get_file_mod_date( '/js/customizer' . $min . '.js' ), true );
		
		wp_enqueue_style( 'cansa-centre-custom-controls-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/customizer' . $min . '.css', null, deneb_get_file_mod_date( '/css/customizer' . $min . '.css' ), 'all' );
 ...
 */
$deneb_customizer = new CansaCentre_Customizer();
