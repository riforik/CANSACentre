...
function deneb_get_sidebar_id() {
	...
	if ( $page_id == $page_for_posts || $page_id == $page_on_front ) {
        $sidebaroptions = get_post_meta( $page_id, 'cansa-centre-sidebar-option', true );
    } elseif ( is_singular() ) {
    	if ( is_attachment() ) {
			$parent 		= $post->post_parent;
			$sidebaroptions = get_post_meta( $parent, 'cansa-centre-sidebar-option', true );

		} else {
			$sidebaroptions = get_post_meta( $post->ID, 'cansa-centre-sidebar-option', true );
		}
	}
...
function deneb_scrollup() {
	...
	echo '<a href="#masthead" id="scrollup" class="backtotop">' . '<span class="screen-reader-text">' . esc_html__( 'Scroll Up', 'cansa-centre' ) . '</span></a>' ;
...
function deneb_get_section_args( $section_name ) {
	$numbers = deneb_gtm( 'cansa_centre_' . $section_name . '_number' );
	...
	for( $i = 0; $i < $numbers; $i++ ) {
		$post__in[] = deneb_gtm( 'cansa_centre_' . $section_name . '_page_' . $i );
	}
...
function deneb_button_border_radius() {
	...
	wp_add_inline_style( 'cansa-centre-style', $css );
}
