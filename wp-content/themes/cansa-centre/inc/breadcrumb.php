...
if ( ! function_exists( 'deneb_breadcrumb' ) ) :
	...
	function deneb_breadcrumb() {
		...
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'cansa-centre' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'cansa-centre' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'cansa-centre' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'cansa-centre' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'cansa-centre' ); // text for an author page
		$text['404']      = esc_html__( '%1$sError 404%2$s', 'cansa-centre' );; // text for the 404 page
		...
		if ( ! is_front_page() ) {
			echo '
			<div id="breadcrumb">
					<div aria-label="' . esc_attr__( 'Breadcrumbs', 'cansa-centre' ) . '" class="breadcrumbs breadcrumb-trail">
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'cansa-centre' ) ) ) ), esc_html( get_the_time( __( 'Y', 'cansa-centre' ) ) ), $content_position++ ) ;

				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'cansa-centre' ) ) ) ), esc_html(get_the_time( __( 'F', 'cansa-centre' ) ) ), $content_position++ );

				echo $before . esc_html( get_the_time( __( 'd', 'cansa-centre' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'cansa-centre' ) ) ) ), esc_html( get_the_time( __( 'Y', 'cansa-centre' ) ) ), $content_position++ ) ;

				echo $before . esc_html( get_the_time( __( 'F', 'cansa-centre' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'cansa-centre' ) ) ) . sprintf( $after, $content_position++ );
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'cansa-centre' ), absint( max( $paged, $page ) ) );
